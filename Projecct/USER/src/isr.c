...
void FLEXCOMM0_DriverIRQHandler(void)
{
    ...
    {
        uart_query(USART_0,&datbuf);		//读取数据
    }
...
//		{
//			uart_query(USART_1,&highbuf);		//读取数据
//			chaoshengbo_flag = 1;
//		}
//		else
//		{
//			uart_query(USART_1,&lowbuf);		//读取数据
//			chaoshengbo_flag = 0;
//		}
//    }
//    
...
//	Flag_10ms = 1;
	...
	Speed_Measure();//测速10ms
	if((i == 1) || (i == 4) || (i == 7) || (i == 10))//30ms
	{
		if(Run_Flag)
		{
			MOTOR_Duty += PID_Cascade(&MOTOR_PID, MOTOR_Speed, Speed_Set);//使用串级增量式PID进行调节
//			MOTOR_Duty += PID_Realize(&MOTOR_PID, MOTOR_Speed, Speed_Set);
			MOTOR_Duty = range_protect(MOTOR_Duty, -9000, 9000);	//限幅保护
			
			MOTOR_Control(MOTOR_Duty);	// 控制左右电机
		}
		else
		{
			if (Stop_Flag)
	...
	if((i == 3) || (i == 6) || (i == 9) || (i == 12))//30ms获取一次阈值
	{
		GetHistGram(image[0]);//获取图像的灰度信息
		yuzhi = OSTUThreshold();//大津法获取图像阈值
		if(yuzhi < 60)
		{
			yuzhi = 60;
		}
	...
	if((i == 2) || (i == 5) || (i == 8) || (i == 11))//30ms获取一次
	{
		Get_Attitude();//姿态解算30ms
	}
}
